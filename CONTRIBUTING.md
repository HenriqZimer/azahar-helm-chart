# Contributing to Azahar Helm Chart
...
helm template azahar chart/

# Install in a test cluster
helm install azahar-test chart/ --dry-run --debug
```
...
- Update `appVersion` in `Chart.yaml` in your PR when pinning the chart to a specific Azahar image
